package io.github.afei.zhihu.view;
...
public class RevealBackgroundView extends View {
    ...
    private static final int FILL_TIME = 800;
...
//        locationY = tapLocationOnScreen[1];
        revealAnimator = ObjectAnimator.ofInt(this, "currentRadius", Math.min(getHeight(), getWidth()), Math.max(getHeight(), getWidth()) * 2).setDuration(FILL_TIME);
    ...
    protected void onDraw(Canvas canvas) {
        if (state == STATE_FINISHED) {
            ...
            canvas.drawCircle(startLocationX, getHeight() * 2, currentRadius, fillPaint);
        }
    }
